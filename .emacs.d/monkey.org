...
Make sure any customisation's are stored in their own file.

#+begin_src emacs-lisp
  (setq custom-file (concat user-emacs-directory "custom.el")
        config-file (concat user-emacs-directory "monkey.org"))
  (load custom-file)
#+end_src

** Package Management
   Define all our package locations, and get use-package to self
   manage.
...
Now, there's an issue where a variable =package-selected-packages=
gets written to =custom.el= everytime I try a package. I do not want
...
then show every installed package as =dependency= instead of
installed. We can correct this by overriding the package ensure
function to our own and fill the variable to correct the listings.

#+begin_src emacs-lisp
  (defun monkey/use-package-ensure-elpa (name args state &optional no-refresh)
  ...
  (setq use-package-ensure-function #'monkey/use-package-ensure-elpa)
#+end_src
...
*** Screen Writing (Youtube scripts)
...
*** Look
Just minor changes for the look of Org mode.

There are a couple of ways within org to emphasize text inline for
*bold*, /italics/, underlined etc. These are set in the text by
...
#+begin_src emacs-lisp
  ...
  (setq org-twitch-file (concat org-directory "/twitch.org"))
#+end_src
