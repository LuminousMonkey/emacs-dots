...
re-rendered."
  (interactive)
  (let* ((posts (org-static-blog-get-post-filenames))
         (previous-post nil)
         (current-post (first posts))
         (next-post (second posts)))
    (while current-post
      (when (org-static-blog-needs-publishing-p current-post)
        (org-static-blog-publish-file current-post
                                      previous-post
                                      next-post))
      (setq previous-post current-post)
      (setq posts (cdr posts))
      (setq current-post (first posts))
      (setq next-post (second posts))))
  ...
  (with-temp-buffer
    ...
    (buffer-substring-no-properties
     (progn
       (goto-char (point-min))
       (if exclude-title
           (progn (search-forward "<h1 class=\"entry-title\">")
                  (search-forward "</h1>"))
         (search-forward "<!-- Start Article -->"))
       (point))
     (progn
       (goto-char (point-max))
       (search-backward "<!-- End Article -->")
       (search-backward "<footer class=\"page-footer\">")
       (point)))))

(defun org-static-blog-close-article (post-filename)
  (concat
   (org-static-blog-get-body post-filename)
   "</article>"))
...
;;;###autoload
(defun org-static-blog-publish-file (post-filename previous-post next-post)
...
The index, archive, tags, and RSS feed are not updated."
  ...
  (org-static-blog-with-find-file
   (org-static-blog-matching-publish-filename post-filename)
   (concat
    (org-static-blog-html-begin-template)
    (org-static-blog-post-preamble post-filename)
    (org-static-blog-render-post-content post-filename)
    (org-static-blog-post-postamble previous-post next-post)
    (org-static-blog-html-end-template))))
...
(defun org-static-blog-html-begin-template ()
  (concat
   "<!DOCTYPE html>\n"
...
(defun org-static-blog-html-end-template ()
  (concat
   "</div>\n"
   "</div>\n"
   "</div>\n"
   "<footer role=\"contentinfo\">\n"
   "<p>Website copyright &copy; 2019 - <a href=\"mailto:mike.aldred@luminousmonkey.org\">Mike Aldred</a> | "
   "<span class=\"credit\">Themed with <a href=\"https://github.com/TheChymera/Koenigspress\">Königspress</a></span>"
   "</footer>\n"
   "</body>\n"
   "</html>\n"))
...
Posts are sorted in descending time."
  ...
  (org-static-blog-with-find-file
   ...
   (concat
    ...
    (apply 'concat (mapcar 'org-static-blog-close-article post-filenames))
    (org-static-blog-html-end-template))))
...
(defun org-static-blog-post-preamble (post-filename)
    ...
    (concat
     ...
     (org-static-blog-get-title post-filename) "</h1>\n"
     "<p class=\"meta\">\n"
...
(defun org-static-blog-post-postamble (previous-post next-post)
  (concat
   "</div>\n"
   ...
   (when previous-post
     (concat
      "<a class=\"basic-alignment left\" href=\""
      (org-static-blog-get-url previous-post)
      "\">"
      "« "
      (org-static-blog-get-title previous-post)
      "</a>\n"))
   (when next-post
     (concat
      "<a class=\"basic-alignment right\" href=\""
      (org-static-blog-get-url next-post)
      "\">"
      (org-static-blog-get-title next-post)
      " »"
      "</a>\n"))
   "</p>\n"
   "</footer>\n"
  ...
  (let ((archive-filename (concat org-static-blog-publish-directory org-static-blog-archive-file))
    ...
    (org-static-blog-with-find-file
     ...
     (concat
      ...
      "<header>\n"
      "<h1 class=\"entry-title\">" (org-static-blog-gettext 'archive) "</h1>\n"
      "</header>\n"
...
archive headline."
  (concat
   ...
   "</h1>\n"
   "<time>\n"
   "<span>" (format-time-string (org-static-blog-gettext 'date-format) (org-static-blog-get-date post-filename)) "</span>\n"
   "</time>\n"
   "</article>\n"))
